...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("randomquark"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::FRQ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert randomquark:// to randomquark:
    //
    //    Cannot handle this later, because randomquark:// will cause Qt to see the part after // as host,
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a randomquark.desktop file to the autostart directory:
