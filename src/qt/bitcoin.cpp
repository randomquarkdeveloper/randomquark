...
int main(int argc, char *argv[])
{
    ...
    // ... then randomquark.conf:
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        // This message can not be translated, as translation is not initialized yet
        // (which not yet possible because lang=XX can be overridden in randomquark.conf in the data directory)
    ...
    try
    {
        ...
        if(AppInit2(threadGroup))
        {
            ...
            // Shutdown the core and its threads, but don't exit FairQuark-Qt here
